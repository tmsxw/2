	...
	protected function configure()
	{
		$this
			...
			->addOption(
				...
				'Log file health check results. This produces no command output and cannot be used with --addon option.'
			);
	}

	protected function execute(InputInterface $input, OutputInterface $output)
	{
		$logResults = $input->getOption('log-results');
		...
		if ($logResults)
		{
			...
			$this->setupAndRunJob('fileCheck', 'XF:FileCheck', [
				'check_id' => $fileCheck->check_id
			], $output);

			$output->writeln(['File checking of all add-ons complete. Check the admin control panel for results.']);
			return 0;
		}
