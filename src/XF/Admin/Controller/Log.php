...
class Log extends AbstractController
{
	...
	public function actionCookieConsent(ParameterBag $params): AbstractReply
	{
		...
		$cookieConsentRepo = $this->repository('XF:CookieConsent');
		...
		{
			$parsedIp = \XF\Util\Ip::parseIpRangeString($limitIp);
			if (!$parsedIp)
			{
				return $this->message(
		...
		if ($limitIp)
		{
			...
			$startRange = \XF\Util\Ip::convertIpStringToBinary(
				$parsedIp['startRange']
			);
			$endRange = \XF\Util\Ip::convertIpStringToBinary(
				$parsedIp['endRange']
			);
			if ($parsedIp['isRange'])
			{
