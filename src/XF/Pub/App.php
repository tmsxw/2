...
class App extends \XF\App
{
	...
	protected function addDefaultNotices(\XF\NoticeList $noticeList, array $pageParams)
	{
		...
		if (
			$cookieConsent['type'] === CookieConsent::MODE_ADVANCED ||
			(!$visitor->user_id && $cookieConsent['type'] === CookieConsent::MODE_SIMPLE)
		)
		{
			$cookieCssClasses = ['notice--primary'];
			if ($cookieConsent['type'] === CookieConsent::MODE_ADVANCED)
			{
